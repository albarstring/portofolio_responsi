    ...
    <div class="container mx-auto max-w-6xl relative z-10">
      ...
      <div class="relative">
        ...
        <div class="space-y-20">
          <div v-for="(edu, index) in pendidikan" :key="index" class="relative flex items-center min-h-[220px]" :class="[
            index % 2 === 0 ? 'flex-row' : 'flex-row-reverse',
            'animate-fade-in-up'
          ]" :style="{ animationDelay: (index * 0.2) + 's' }">
            <!-- Timeline dot -->
            <div
              class="absolute left-1/2 transform -translate-x-1/2 w-6 h-6 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full border-4 border-gray-900 z-10 shadow-lg">
            </div>

            <!-- Content -->
            <div
              class="w-5/12 max-w-[90vw] break-words p-6 md:p-8 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl hover:bg-white/10 transition-all duration-300 hover:scale-105 min-h-[180px] flex flex-col justify-center shadow-lg">
              <div class="text-sm text-purple-400 font-semibold mb-2 tracking-wide">{{ edu.period }}</div>
              <h3 class="text-lg md:text-xl font-bold text-white mb-2 text-center">{{ edu.institution }}</h3>
