<template>
  ...
  <section id="profile" class="min-h-screen flex items-center justify-center px-6 relative" style="margin-top: 6rem;">
    <div class="flex flex-col-reverse md:flex-row items-center justify-center w-full max-w-6xl mx-auto gap-12">
      ...
      <div class="flex-1 text-center md:text-left">
        ...
        <div class="animate-fade-in-up" style="animation-delay: 0.2s">
          ...
          <h2 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-300 mb-6">
        ...
        <!-- Description -->
        <p class="text-lg md:text-xl text-gray-800 dark:text-gray-300 mb-8 max-w-2xl mx-auto md:mx-0 leading-relaxed animate-fade-in-up"
        ...
        <!-- CTA Buttons -->
        <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-start animate-fade-in-up"
          style="animation-delay: 0.6s">
          <button @click="scrollToSection('proyek')"
            class="group relative px-8 py-4 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-full font-semibold transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-purple-500/25">
          ...
          <a href="/CV_Albar.pdf" download
            class="px-8 py-4 border border-purple-500 text-gray-800 dark:text-white dark:border-purple-400 rounded-full font-semibold hover:bg-purple-100 dark:hover:bg-purple-400/10 transition-all duration-300 hover:scale-105">
            Download CV
          </a>
        </div>

        <!-- Social Links -->
        <div class="flex justify-center md:justify-start space-x-6 mt-12 animate-fade-in-up"
          style="animation-delay: 0.8s">
          <a v-for="social in boxSocialLinks" :key="social.name" :href="social.url" target="_blank" rel="noopener"
            class="w-12 h-12 bg-black/5 dark:bg-white/5 backdrop-blur-sm border border-black/10 dark:border-white/10 rounded-full flex items-center justify-center text-xl text-black dark:text-white hover:bg-black/10 dark:hover:bg-white/10 hover:scale-110 hover:text-purple-400 transition-all duration-300">
    ...
    <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
      <div class="w-6 h-10 border-2 border-gray-800/30 dark:border-white/30 rounded-full flex justify-center">
        <div class="w-1 h-3 bg-gray-800/60 dark:bg-white/60 rounded-full mt-2 animate-pulse"></div>
      </div>
    </div>
  </section>
</template>
...
html:not(.dark) .hero-bg {
  background: #f9fafb;
  background-image:
    radial-gradient(ellipse at 20% 30%, rgba(128, 0, 255, 0.05) 0, transparent 60%),
    radial-gradient(ellipse at 80% 20%, rgba(0, 180, 255, 0.05) 0, transparent 60%),
    radial-gradient(ellipse at 50% 80%, rgba(255, 0, 128, 0.05) 0, transparent 60%);
}
