    ...
    <div class="container mx-auto max-w-6xl">
      ...
      <div class="space-y-8">
        <div v-for="(exp, index) in experience" :key="index"
             class="group p-8 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl hover:bg-white/10 transition-all duration-300 hover:scale-[1.02] animate-slide-in-left"
             :class="{ 'animate-slide-in-left': experienceVisible }"
             :style="{ animationDelay: (index * 0.2) + 's' }">
          
          <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
            <div>
              <h3 class="text-2xl font-bold text-white mb-2">{{ exp.position }}</h3>
              <p class="text-purple-400 font-semibold text-left">{{ exp.company }}</p>
            </div>
            <div class="text-gray-400 mt-2 md:mt-0">
              <span class="bg-purple-500/20 text-purple-300 px-3 py-1 rounded-full text-sm">{{ exp.period }}</span>
            </div>
          </div>
          
          <p class="text-gray-300 mb-4 text-left">{{ exp.description }}</p>
          
          <div class="flex flex-wrap gap-2">
            <span v-for="tech in exp.technologies" :key="tech"
                  class="px-3 py-1 bg-blue-500/20 text-blue-300 rounded-full text-xs hover:bg-blue-500/30 transition-colors duration-300">
              {{ tech }}
            </span>
          </div>
        </div>
